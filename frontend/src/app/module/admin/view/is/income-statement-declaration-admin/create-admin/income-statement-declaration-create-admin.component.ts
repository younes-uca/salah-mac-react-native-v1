...
@Component({
  selector: 'app-income-statement-declaration-create-admin',
  templateUrl: './income-statement-declaration-create-admin.component.html'
})
export class IncomeStatementDeclarationCreateAdminComponent extends AbstractCreateController<IncomeStatementDeclarationDto, IncomeStatementDeclarationCriteria, IncomeStatementDeclarationService>  implements OnInit {
    ...
    const isPermistted = await this.roleService.isPermitted('CorporateTaxRate', 'add');
    if(isPermistted) {
         this.corporateTaxRate = new CorporateTaxRateDto();
