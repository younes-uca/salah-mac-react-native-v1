...
@Component({
  selector: 'app-accounting-operation-edit-admin',
  templateUrl: './accounting-operation-edit-admin.component.html'
})
export class AccountingOperationEditAdminComponent extends AbstractEditController<AccountingOperationDto, AccountingOperationCriteria, AccountingOperationService>   implements OnInit {
    ...
    public prepareEdit() {
        this.item.accountingOperationDate = this.accountingOperationService.format(this.item.accountingOperationDate);
    }
   ...
   public async openCreateAccountingOperationStatus(accountingOperationStatus: string) {
        const isPermistted = await this.roleService.isPermitted('AccountingOperationStatus', 'edit');
        if(isPermistted) {
             this.accountingOperationStatus = new AccountingOperationStatusDto();
   ...
   public async openCreateAccountingOperationType(accountingOperationType: string) {
        const isPermistted = await this.roleService.isPermitted('AccountingOperationType', 'edit');
        if(isPermistted) {
             this.accountingOperationType = new AccountingOperationTypeDto();
   ...
   public async openCreateAccountingAccount(accountingAccount: string) {
        const isPermistted = await this.roleService.isPermitted('AccountingAccount', 'edit');
        if(isPermistted) {
             this.accountingAccount = new AccountingAccountDto();
