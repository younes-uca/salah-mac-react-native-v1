...
@Component({
  selector: 'app-balance-sheet-edit-admin',
  templateUrl: './balance-sheet-edit-admin.component.html'
})
export class BalanceSheetEditAdminComponent extends AbstractEditController<BalanceSheetDto, BalanceSheetCriteria, BalanceSheetService>   implements OnInit {
   ...
   public async openCreateBalanceSheetStatus(balanceSheetStatus: string) {
        const isPermistted = await this.roleService.isPermitted('BalanceSheetStatus', 'edit');
        if(isPermistted) {
             this.balanceSheetStatus = new BalanceSheetStatusDto();
