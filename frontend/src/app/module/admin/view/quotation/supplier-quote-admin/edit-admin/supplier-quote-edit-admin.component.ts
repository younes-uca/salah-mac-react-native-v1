...
@Component({
  selector: 'app-supplier-quote-edit-admin',
  templateUrl: './supplier-quote-edit-admin.component.html'
})
export class SupplierQuoteEditAdminComponent extends AbstractEditController<SupplierQuoteDto, SupplierQuoteCriteria, SupplierQuoteService>   implements OnInit {
    ...
    public prepareEdit() {
        this.item.quoteDate = this.supplierQuoteService.format(this.item.quoteDate);
    }



    public validateSupplierQuoteItems(){
   ...
   public addSupplierQuoteItems() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.supplierQuoteItemsElement.id == null){
                this.item.supplierQuoteItems.push(this.supplierQuoteItemsElement);
            }else{
                const index = this.item.supplierQuoteItems.findIndex(e => e.id == this.supplierQuoteItemsElement.id);
                this.item.supplierQuoteItems[index] = this.supplierQuoteItemsElement;
            }
