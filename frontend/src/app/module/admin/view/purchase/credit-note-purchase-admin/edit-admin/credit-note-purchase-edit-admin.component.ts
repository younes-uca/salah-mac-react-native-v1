...
@Component({
  selector: 'app-credit-note-purchase-edit-admin',
  templateUrl: './credit-note-purchase-edit-admin.component.html'
})
export class CreditNotePurchaseEditAdminComponent extends AbstractEditController<CreditNotePurchaseDto, CreditNotePurchaseCriteria, CreditNotePurchaseService>   implements OnInit {
    ...
    public prepareEdit() {
        this.item.creditNoteDate = this.creditNotePurchaseService.format(this.item.creditNoteDate);
    }



    public validateCreditNotePurchaseItems(){
   ...
   public addCreditNotePurchaseItems() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.creditNotePurchaseItemsElement.id == null){
                this.item.creditNotePurchaseItems.push(this.creditNotePurchaseItemsElement);
            }else{
                const index = this.item.creditNotePurchaseItems.findIndex(e => e.id == this.creditNotePurchaseItemsElement.id);
                this.item.creditNotePurchaseItems[index] = this.creditNotePurchaseItemsElement;
            }
   ...
   public async openCreateCreditNotePurchaseStatus(creditNotePurchaseStatus: string) {
        const isPermistted = await this.roleService.isPermitted('CreditNotePurchaseStatus', 'edit');
        if(isPermistted) {
             this.creditNotePurchaseStatus = new CreditNotePurchaseStatusDto();
