...
@Component({
  selector: 'app-bank-check-owner-edit-admin',
  templateUrl: './bank-check-owner-edit-admin.component.html'
})
export class BankCheckOwnerEditAdminComponent extends AbstractEditController<BankCheckOwnerDto, BankCheckOwnerCriteria, BankCheckOwnerService>   implements OnInit {
   ...
   public async openCreateCheckOwner(checkOwner: string) {
        const isPermistted = await this.roleService.isPermitted('CheckOwner', 'edit');
        if(isPermistted) {
             this.checkOwner = new CheckOwnerDto();
   ...
   public async openCreateBank(bank: string) {
        const isPermistted = await this.roleService.isPermitted('Bank', 'edit');
        if(isPermistted) {
             this.bank = new BankDto();
