...
@Component({
  selector: 'app-vat-declaration-edit-admin',
  templateUrl: './vat-declaration-edit-admin.component.html'
})
export class VatDeclarationEditAdminComponent extends AbstractEditController<VatDeclarationDto, VatDeclarationCriteria, VatDeclarationService>   implements OnInit {
    ...
    public prepareEdit() {
        this.item.declarationDate = this.vatDeclarationService.format(this.item.declarationDate);
    }
   ...
   public async openCreateLatePaymentRate(latePaymentRate: string) {
        const isPermistted = await this.roleService.isPermitted('LatePaymentRate', 'edit');
        if(isPermistted) {
             this.latePaymentRate = new LatePaymentRateDto();
