...
@Component({
  selector: 'app-return-sale-edit-admin',
  templateUrl: './return-sale-edit-admin.component.html'
})
export class ReturnSaleEditAdminComponent extends AbstractEditController<ReturnSaleDto, ReturnSaleCriteria, ReturnSaleService>   implements OnInit {
    ...
    public prepareEdit() {
        this.item.returnDate = this.returnSaleService.format(this.item.returnDate);
    }



    public validateReturnSaleItems(){
   ...
   public addReturnSaleItems() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.returnSaleItemsElement.id == null){
                this.item.returnSaleItems.push(this.returnSaleItemsElement);
            }else{
                const index = this.item.returnSaleItems.findIndex(e => e.id == this.returnSaleItemsElement.id);
                this.item.returnSaleItems[index] = this.returnSaleItemsElement;
            }
   ...
   public async openCreateReturnStatus(returnStatus: string) {
        const isPermistted = await this.roleService.isPermitted('ReturnStatus', 'edit');
        if(isPermistted) {
             this.returnStatus = new ReturnStatusDto();
   ...
   public async openCreateSale(sale: string) {
        const isPermistted = await this.roleService.isPermitted('Sale', 'edit');
        if(isPermistted) {
             this.sale = new SaleDto();
