...
@Component({
  selector: 'app-order-payment-edit-admin',
  templateUrl: './order-payment-edit-admin.component.html'
})
export class OrderPaymentEditAdminComponent extends AbstractEditController<OrderPaymentDto, OrderPaymentCriteria, OrderPaymentService>   implements OnInit {
    ...
    public prepareEdit() {
        this.item.paymentDate = this.orderPaymentService.format(this.item.paymentDate);
    }
   ...
   public async openCreateOrderStatusOrder(orderStatusOrder: string) {
        const isPermistted = await this.roleService.isPermitted('OrderStatusOrder', 'edit');
        if(isPermistted) {
             this.orderStatusOrder = new OrderStatusOrderDto();
